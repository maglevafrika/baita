...
import { PlusCircle, Plane, CheckCircle, XCircle, Loader2, User, Building, Users } from "lucide-react";
...
function AddLeaveDialog({ onLeaveAdded }: { onLeaveAdded: () => void }) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild><Button><PlusCircle className="mr-2" /> New Leave Request</Button></DialogTrigger>
      <DialogContent>
        <DialogHeader><DialogTitle>New Leave Request</DialogTitle></DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <FormField control={form.control} name="type" render={({ field }) => (
                <FormItem><FormLabel>Request For</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}>
            ...
            {leaveType && (
                 <FormField control={form.control} name="personId" render={({ field }) => (
                    <FormItem><FormLabel>{leaveType === 'student' ? 'Student' : 'Teacher'}</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}>
                        <FormControl><SelectTrigger><SelectValue placeholder={`Select a ${leaveType}...`} /></SelectTrigger></FormControl>
                        <SelectContent><ScrollArea className="h-48">{personOptions.map(p => <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}</ScrollArea></SelectContent>
                    </Select><FormMessage /></FormItem>
                )} />
            )}
            ...
            />
             <FormField control={form.control} name="reason" render={({ field }) => (
                <FormItem><FormLabel>Reason</FormLabel><FormControl><Textarea placeholder="Reason for leave..." {...field} /></FormControl><FormMessage /></FormItem>
            )} />
            <DialogFooter>
              <Button type="submit" disabled={isLoading}>{isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Submit Request</Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
...
function TransferStudentsDialog({ leaveRequest, onOpenChange, onApprovalSuccess }: { leaveRequest: Leave, onOpenChange: (open: boolean) => void, onApprovalSuccess: () => void }) {
  ...
  const affectedStudents = useMemo(() => {
    const affected: { studentId: string, studentName: string, sessionId: string, day: string, newTeacherId: string }[] = [];
    if (!leaveRequest || leaveRequest.type !== 'teacher') return [];

    const leaveInterval = { start: new Date(leaveRequest.startDate), end: new Date(leaveRequest.endDate) };

    for (const semester of semesters) {
        if (!isWithinInterval(new Date(), { start: new Date(semester.startDate), end: new Date(semester.endDate) })) continue;
        ...
        Object.entries(teacherSchedule).forEach(([day, sessions]) => {
            ...
            for (const session of sessions) {
                for (const student of session.students) {
                    const studentIsAffected = leaveDays.some(leaveDay => leaveDay.getDay() === (dayIndex + 1) % 7);
                    if(studentIsAffected && !affected.find(a => a.studentId === student.id && a.sessionId === session.id)) {
                         affected.push({ studentId: student.id, studentName: student.name, sessionId: session.id, day, newTeacherId: '' });
                    }
                }
            }
        });
  ...
  const onSubmit = async (data: TransferFormValues) => {
    ...
    try {
      ...
      const semester = semesters.find(s => isWithinInterval(new Date(), { start: new Date(s.startDate), end: new Date(s.endDate) }));
      ...
      for (const transfer of data.transfers) {
        ...
        if (oldSessionIndex !== -1) {
            oldTeacherSessions[oldSessionIndex].students = oldTeacherSessions[oldSessionIndex].students.filter((s: StudentProfile) => s.id !== transfer.studentId);
        }
        ...
        const studentData = { id: transfer.studentId, name: transfer.studentName, attendance: null, pendingRemoval: false };
        ...
        const studentProfile = await updateStudent(transfer.studentId, { 
            enrolledIn: students.find(s => s.id === transfer.studentId)!.enrolledIn.map(e => 
                e.sessionId === transfer.sessionId ? { ...e, teacher: newTeacher.name } : e
            )
        });
      }

      await updateSemester(semester.id, { masterSchedule: updatedMasterSchedule });
      await onApprovalSuccess();
      toast({ title: "Students Transferred", description: "The students have been moved to their new schedules." });
      onOpenChange(false);
    ...
    <Dialog open={true} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-2xl">
            <DialogHeader>
                ...
                <DialogDescription>The following students are affected by this leave. Please reassign them to a new teacher for the duration.</DialogDescription>
            </DialogHeader>
            <Form {...form}>
            ...
            <ScrollArea className="h-72 my-4">
                ...
                {fields.map((field, index) => (
                    <div key={field.id} className="p-4 border rounded-md">
                        ...
                        <p className="text-sm text-muted-foreground">Session on {field.day}</p>
                         <FormField
                            ...
                            render={({ field }) => (
                                <FormItem className="mt-2"><FormLabel>New Teacher</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}>
                                    <FormControl><SelectTrigger><SelectValue placeholder="Select a new teacher..." /></SelectTrigger></FormControl>
                                    <SelectContent>
                                      {teachers.filter(t => t.id !== leaveRequest.personId).map(t => <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}
                                    </SelectContent>
                                </Select><FormMessage /></FormItem>
                            )}
                         />
                 ...
                 {fields.length === 0 && <p className="text-center text-muted-foreground">No students are affected during this leave period.</p>}
                </div>
            </ScrollArea>
             <DialogFooter>
                <Button type="button" variant="ghost" onClick={() => onOpenChange(false)}>Cancel</Button>
                <Button type="submit" disabled={isLoading}>{isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Confirm Transfers & Approve</Button>
            </DialogFooter>
            </form>
            </Form>
        </DialogContent>
...
export default function LeavesPage() {
  ...
  const handleApprove = async (leave: Leave) => {
    if (leave.type === 'teacher') {
        ...
        toast({ title: "Leave Approved", description: `The leave request for ${leave.personName} has been approved.`});
    }
  };
  
  const handleDeny = async (leave: Leave) => {
    await updateLeave(leave.id, { status: 'denied' });
    toast({ title: "Leave Denied", description: `The leave request for ${leave.personName} has been denied.`, variant: 'destructive'});
  };
                ...
                <Card key={leave.id}>
                    <CardHeader>
                        <CardTitle className="flex justify-between items-start">
                            ...
                            <Badge variant={getStatusVariant(leave.status)} className="capitalize">{leave.status}</Badge>
                        </CardTitle>
                        <CardDescription className="flex items-center gap-2">
                           {leave.type === 'student' ? <User className="h-4 w-4" /> : <Building className="h-4 w-4" />} 
                           <span className="capitalize">{leave.type}</span>
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 text-sm">
                        <p><strong>Dates:</strong> {format(new Date(leave.startDate), 'PPP')} to {format(new Date(leave.endDate), 'PPP')}</p>
                        <p><strong>Reason:</strong> {leave.reason}</p>
                    </CardContent>
                    {leave.status === 'pending' && (
                        <CardFooter className="flex gap-2">
                            <Button size="sm" className="w-full" onClick={() => handleApprove(leave)}><CheckCircle className="mr-2" /> Approve</Button>
                            <Button size="sm" variant="destructive" className="w-full" onClick={() => handleDeny(leave)}><XCircle className="mr-2"/> Deny</Button>
                        </CardFooter>
                    )}
                </Card>
              ))}
